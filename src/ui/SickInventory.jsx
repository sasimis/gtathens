...
const SickInventory = () => {
  ...
  // Owned ids (fists is implicit) — memoised so effects below can use it.
  const ownedIds = React.useMemo(() => new Set(weapons.map((w) => w.id)), [weapons])
  ...
  useEffect(() => {
    ...
    const onKey = (e) => {
      ...
      e.preventDefault()
      const order = WEAPON_ORDER.filter((id) => id === 'fists' || ownedIds.has(id))
